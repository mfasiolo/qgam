...
qgam <- function(form, data, tau, err = 0.01, ncores = 1, control = list(), controlGam = list())
{
  # Initial Gaussianfit
  ...
  qfit <- lapply(1:nt, function(ii){
    ...
    lam <- err * sqrt(2*pi*gausFit$sig2) / (2*log(2)*exp(learn$lsigma[ii])) 
    
    out <-  gam(form, family = logF(tau = tau[ii], lam = lam, theta = learn$lsigma[ii]), data = data)
