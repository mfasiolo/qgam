...
#       stop("Matteo: I can't calculate the saturated likelihood")
  ...
  ll <- function(y, X, coef, wt, family, deriv=0, d1b=0, d2b=0, Hp=NULL, rank=0, fh=NULL, D=NULL) {
    ...
    if (deriv>0) {
      ...
      Dm <- 1/(del*sig*sqrt(z^2+1))*(del*(De)+z/sqrt(z^2+1))
      Dt <- z*del*sig*Dm - 1
      Dp <- Dt + 1 - del*asinh(z)*De
      ...
      Dme <- (sech(g)^2 - cosh(2*g)) / (sig*sqrt(z^2+1))
      Dte <- z*del*sig*Dme
      Dmm <- Dme/(sig*sqrt(z^2+1)) + z*De/(sig^2*del*(z^2+1)^(3/2)) + (z^2-1)/(del*sig*del*sig*(z^2+1)^2)
      Dmt <- z*sig*del*Dmm - Dm
      Dee <- -2*cosh(g)^2 + sech(g)^2 + 1 
      Dtt <-  z*del*sig*Dmt
      Dep <- Dte - del*asinh(z)*Dee
      Dmp <- Dmt + De/(sig*sqrt(z^2+1)) - del*asinh(z)*Dme
      Dtp <- z*del*sig*Dmp
      Dpp <- Dtp - del*asinh(z)*Dep + del*(z/sqrt(z^2+1)-asinh(z))*De
    ...
    if (deriv>1) {
      ...
      Dmee <- Deee/(sig*sqrt(z^2+1))
      Dmme <- Dmee/(sig*sqrt(z^2+1)) + z*Dee/(sig*sig*del*(z^2+1)^(3/2))
      Dmmm <- 2*z*Dme/(sig*sig*del*sqrt(z^2+1)^3) + Dmme/(sig*sqrt(z^2+1)) + 
              (2*z^2-1)*De/(sig^3*del^2*sqrt(z^2+1)^5) + 2*z*(z^2-3)/((sig*del)^3*(z^2+1)^3)
      Dmmt <- z*sig*del*Dmmm - 2*Dmm
      Dtee <- z*sig*del*Dmee
      Dmte <- z*sig*del*Dmme - Dme
      Dtte <- z*sig*del*Dmte
      Dmtt <- z*sig*del*Dmmt - Dmt
      Dttt <- z*sig*del*Dmtt
      Dmep <- Dmte + Dee/(sig*sqrt(z^2+1)) - del*asinh(z)*Dmee
      Dtep <- z*sig*del*Dmep
      Deep <- Dtee - del*asinh(z)*Deee
      Depp <- Dtep - del*asinh(z)*Deep + del*( z/sqrt(z^2+1)-asinh(z) )*Dee
      Dmmp <- Dmmt + 2*Dme/(sig*sqrt(z^2+1)) + z*De/(del*sig*sig*sqrt(z^2+1)^3) - del*asinh(z)*Dmme
      Dmtp <- z*sig*del*Dmmp - Dmp
      Dttp <- z*sig*del*Dmtp
      Dmpp <- Dmtp + Dep/(sig*sqrt(z^2+1)) + z^2*De/(sig*sqrt(z^2+1)^3) - 
              del*asinh(z)*Dmep + del*Dme*(z/sqrt(z^2+1) - asinh(z))
      Dtpp <- z*sig*del*Dmpp
      Dppp <- Dtpp - del*asinh(z)*Depp + del*(z/sqrt(z^2+1)-asinh(z))*(2*Dep + De) + del*(z/sqrt(z^2+1))^3 * De
