...
fit <- qgam(accel~s(times, k=20, bs="ad"), 
            data = mcycle, 
            qu = 0.8, 
            control = list("tol" = 0.02)) # <- sloppy tolerance to speed-up calibration 
...
Notice that here two quantile curves cross on the far-right of the plot. Given that each quantile model is fitted
independently, there is nothing preventing this from happening. This is a well-known problem of quantile regression. 
...
We fit a constant model to the median and we use `cqcheck` produce a diagnostic plot. 
```{r c2}
qu <- 0.5
